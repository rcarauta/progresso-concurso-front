...
const OrderDisciplina: React.FC = () => {
    ...
    const handleOrdenacaoChange = (e: React.ChangeEvent<HTMLInputElement>, disciplinaId: number) => {
      const value = parseInt(e.target.value, 10);
      
      if (value >= 1 && value <= 20) {
        setOrdens((prevOrdens) => ({
          ...prevOrdens,
          [disciplinaId]: value, // Atualizando o valor de ordem da disciplina específica
        }));
      }
    };
  
    // Função que envia a ordenação para o Redux
    const handleOrdenar = (disciplinaId: number) => {
      const ordem = ordens[disciplinaId]; // Pegando o valor de ordem para a disciplina específica
      if (ordem !== undefined && concursoId) {
        dispatch(ordenateDisciplina({ idConcurso: concursoId, disciplinaId, numeroOrdem: ordem }));
      }
    };
                    ...
                    <Row className="align-items-center">
                      <Col xs={6}>
                        <Form.Control
                          ...
                          min="1"
                          max="20"
                          value={ordens[disciplina.id] || ''} // Pegando a ordem específica da disciplina
                          onChange={(e) => handleOrdenacaoChange(e, disciplina.id)} 
                          className="mb-2"
